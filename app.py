...
# Đảm bảo import đúng cách cho TensorFlow
...
def load_model_once():
    ...
    with model_lock:
        ...
        try:
            ...
            model = model_local
            # Tăng cường kiểm tra model sau khi load
            if model is not None:
                print("✅ Model EfficientNet đã load thành công.")
            else:
                raise Exception("Load model thành công nhưng model object bị None.")
            return model
...
# Giữ nguyên logic sửa lỗi dùng biến 'summary'
# ==============================
...
# ==============================
# EMR Prediction (Dự đoán hình ảnh) - ĐÃ TẬP TRUNG SỬA LỖI
# ==============================
...
def emr_prediction():
    ...
    if request.method == "POST":
        ...
        if file and file.filename:
            ...
            # Tinh chỉnh: Thêm kiểm tra định dạng file ảnh
            if not filename.lower().endswith(('.png', '.jpg', '.jpeg', '.gif', '.bmp')):
                flash("Chỉ chấp nhận file ảnh (PNG, JPG, JPEG, GIF, BMP) để dự đoán.", 'danger')
                # Trả về trang mà không dự đoán
            ...
            try:
                ...
                mdl = load_model_once()
                if mdl:
                    # Tinh chỉnh: Mở ảnh trong khối try/except riêng biệt để bắt lỗi mở file
                    ...
                    raise Exception("Mô hình AI không khả dụng.")
            
            except Exception as e:
                # Bắt tất cả các lỗi xảy ra trong quá trình save, mở ảnh hoặc dự đoán
                error_message = f"Lỗi dự đoán: {e}"
                print(f"Lỗi dự đoán: {e}")
                flash(error_message, 'danger')
                prediction_result = {
                    "result": error_message,
                    "probability": 0,
                    "image_url": uploaded_image_url # Vẫn cố gắng hiển thị ảnh nếu URL đã được tạo
                }
    
    # Truyền filename (dù có lỗi hay không) và prediction_result
...
try:
    ...
    # Tắt logging của TensorFlow để console sạch hơn
